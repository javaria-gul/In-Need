...
async function bootstrap() {
  ...
  if (configService.swagger.enabled) {
    ...
    console.log(`📚 Swagger API documentation: http://192.168.1.11:${configService.port}/api`);
  }
  ...
  const corsOrigins = configService.isDevelopment ? 'All 192.168.1.11 ports (development mode)' : configService.corsOrigins.join(', ');
